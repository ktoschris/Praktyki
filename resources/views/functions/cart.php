...
<div>
    <div>
        <h22>Items in cart: <span data-bind="text: getProductCount"></span></h22>
    </div>
    <br>
    <table >
        <thead>
...
<script type="text/javascript">
    ...
    var Cart = function (products) {
        ...
        self.addOneNewProductToList = function (alias, name, price, qt) {
            //self.products().push(new Product(alias, name, price, parseInt(self.newProductQt())));
            //self.products.valueHasMutated();

            var flaga=1;
            var prod=new Product(alias, name, price, 1);
            for(var i in self.products()){
                if(prod.name() === self.products()[i].name()){
                    self.products()[i].increasebyQt(1);
                    flaga=0;
                    break;
                }
